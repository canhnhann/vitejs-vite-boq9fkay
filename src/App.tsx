...
const initialData = {
  ...
  goals: [
    ...
    {
      id: 2,
      text: 'Hoàn thành khóa học lập trình',
      deadline: '2025-07-15',
      done: false,
    },
  ],
...
export default function App() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      supabase
        .from('tracker_data')
        .upsert(
          { user_id: USER_ID, data, updated_at: new Date() },
          { onConflict: 'user_id' }
        )
        .then(({ error }) => {
  ...
  const [newOutingDate, setNewOutingDate] = useState(
    new Date().toISOString().slice(0, 10)
  );
  ...
  const doneLessons = data.lessons.filter(
    (l) => l.status === 'hoàn thành'
  ).length;
  const totalSubjectItems = data.subjects.reduce(
    (a, s) => a + s.items.length,
    0
  );
  const doneSubjectItems = data.subjects.reduce(
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          >
            {[
              {
                label: 'Bài học',
                value: `${doneSubjectItems}/${totalSubjectItems}`,
                icon: '📚',
              },
              {
                label: 'Việc xong',
                value: `${doneTodos}/${data.todos.length}`,
                icon: '✅',
              },
              {
                label: 'Mục tiêu',
                value: `${doneGoals}/${data.goals.length}`,
                icon: '🎯',
              },
              {
                label: 'Thời gian',
              ...
              {
                label: 'Bài soạn',
                value: `${doneLessons}/${data.lessons.length}`,
                icon: '📝',
              },
            ].map((s) => (
              <div
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px' }}>
                  ...
                  {isOpen && (
                    ...
                    >
                      {s.items.length === 0 && (
                        ...
                        >
                          Chưa có bài học nào. Thêm bài bên dưới!
                        </div>
                      )}
                      {s.items.map((item) => (
                        <div
        ...
        {tab === 1 && (
          <div>
                  ...
                  {items.map((t) => (
                    <div
                      ...
                      style={{
                        ...
                        alignItems: 'center',
                        gap: 10,
                        borderLeft: `3px solid ${PRIORITY_COLOR[p]}`,
                      }}
                    >
                      <input
                        type="checkbox"
                      ...
                      <button
                        onClick={() =>
                          setData((d) => ({
                            ...d,
                            todos: d.todos.filter((x) => x.id !== t.id),
                          }))
                        }
                        style={{
                          background: 'none',
            ...
            {data.todos.length === 0 && (
              <Empty text="Chưa có việc cần làm nào" />
            )}
          </div>
        )}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: 16,
                    marginBottom: 12,
                    borderLeft: `3px solid ${g.done ? '#10b981' : '#38bdf8'}`,
                  }}
                >
                  <div
                    style={{
        ...
        {tab === 3 && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              ...
              <StatCard
                label="Số buổi học"
                value={data.studyLog.length}
                color="#a78bfa"
              />
              <StatCard
                label="TB / buổi"
                value={
                  data.studyLog.length
                    ? `${Math.round(totalStudyMinutes / data.studyLog.length)}m`
                    : '—'
                }
                color="#34d399"
              />
              ...
              >
                ...
                <button
                  onClick={() =>
                    setData((d) => ({
                      ...d,
                      studyLog: d.studyLog.filter((x) => x.id !== l.id),
                    }))
                  }
                  style={{
                    background: 'none',
            ...
            {data.studyLog.length === 0 && (
              <Empty text="Chưa có buổi học nào được ghi lại" />
            )}
          </div>
        )}
            ...
            >
              <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 6 }}>
                ...
                <b style={{ color: '#10b981' }}>
                  {doneLessons}/{data.lessons.length}
                </b>
              </div>
              <div
                style={{ background: '#0f172a', borderRadius: 999, height: 8 }}
              >
                <div
                  style={{
                    width: data.lessons.length
                      ? `${(doneLessons / data.lessons.length) * 100}%`
                      : '0%',
                    height: '100%',
                    background: '#10b981',
                    borderRadius: 999,
            ...
            {data.lessons.map((l) => (
              <div
                ...
                style={{
                  ...
                  alignItems: 'center',
                  gap: 10,
                  borderLeft: `3px solid ${STATUS_COLOR[l.status]}`,
                }}
              >
                <div style={{ flex: 1 }}>
                ...
                <span
                  style={{
                    ...
                    padding: '2px 8px',
                    borderRadius: 999,
                    background: STATUS_COLOR[l.status] + '33',
                ...
                <button
                  onClick={() =>
                    setData((d) => ({
                      ...d,
                      lessons: d.lessons.filter((x) => x.id !== l.id),
                    }))
                  }
                  style={{
                    background: 'none',
        ...
        {tab === 5 &&
          (() => {
            const totalIncome = data.finance.income.reduce(
              (a, b) => a + (Number(b.amount) || 0),
              0
            );
            const totalDebt = data.finance.debt.reduce(
              (a, b) => a + (Number(b.amount) || 0),
              0
            );
            const paidDebt = data.finance.debt
              .filter((d) => d.paid)
              .reduce((a, b) => a + (Number(b.amount) || 0), 0);
            const totalExpenses = data.finance.expenses.reduce(
              (a, b) => a + (Number(b.amount) || 0),
              0
            );
            const totalOutings = data.finance.outings.reduce(
              (a, b) => a + (Number(b.amount) || 0),
              0
            );
            const remaining =
              totalIncome - totalDebt - totalExpenses - totalOutings;
            const fmt = (n) => n.toLocaleString('vi-VN') + 'đ';

            const updateFinance = (key, val) =>
              setData((d) => ({ ...d, finance: { ...d.finance, [key]: val } }));
              ...
              <div>
                ...
                >
                  ...
                  <SummaryCard
                    label="Còn lại"
                    value={fmt(remaining)}
                    color={remaining >= 0 ? '#38bdf8' : '#f43f5e'}
                    icon="🏦"
                  />
                </div>
                ...
                >
                  {data.finance.debt.map((item) => (
                    <div
                      key={item.id}
                      style={{ ...finRowStyle, opacity: item.paid ? 0.5 : 1 }}
                    >
                      <input
                        type="checkbox"
                        checked={item.paid}
                        onChange={() =>
                          updateFinance(
                            'debt',
                            data.finance.debt.map((x) =>
                              x.id === item.id ? { ...x, paid: !x.paid } : x
                            )
                          )
                        }
                        style={{
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            updateFinance(
                              'debt',
                              data.finance.debt.filter((x) => x.id !== item.id)
                            )
                          }
                          style={delBtn}
                        >
                ...
                >
                          ...
                          {item.min > 0 && (
                            <span
                              style={{
                                fontSize: 11,
                                marginLeft: 8,
                                color: belowMin ? '#f43f5e' : '#10b981',
                                fontWeight: 600,
                              }}
                            >
                              {belowMin
                                ? `⚠ tối thiểu ${item.min.toLocaleString(
                                    'vi-VN'
                                  )}đ`
                                : '✓ đủ'}
                            </span>
                        ...
                        >
                          <input
                            ...
                            style={{
                              ...
                              fontSize: 13,
                              textAlign: 'right',
                              borderColor: belowMin ? '#f43f5e' : '#334155',
                            }}
                          />
                          <span style={{ fontSize: 12, color: '#475569' }}>
                            đ
                  ...
                  >
                    ...
                    <input
                      type="number"
                      value={newExpenseMin}
                      onChange={(e) => setNewExpenseMin(e.target.value)}
                ...
                >
                  ...
                  >
                    <input
                      type="date"
                      value={newOutingDate}
                      onChange={(e) => setNewOutingDate(e.target.value)}
                      style={{
                        ...inputStyle,
                        fontSize: 13,
                        padding: '6px 10px',
                    ...
                    <input
                      value={newOutingNote}
                      onChange={(e) => setNewOutingNote(e.target.value)}
                      placeholder="Ghi chú (đi đâu, làm gì...)"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && newOutingAmount) {
                          updateFinance('outings', [
                            ...data.finance.outings,
                            {
                              id: Date.now(),
                              date: newOutingDate,
                              note: newOutingNote.trim(),
                              amount: Number(newOutingAmount),
                            },
                          ]);
                          setNewOutingNote('');
                          setNewOutingAmount('');
                    ...
                    <input
                      type="number"
                      value={newOutingAmount}
                      onChange={(e) => setNewOutingAmount(e.target.value)}
                      placeholder="Số tiền..."
                      style={{
                        ...inputStyle,
                        width: 110,
                  ...
                  {[...data.finance.outings]
                    ...
                    .map((o) => (
                      <div
                        key={o.id}
                        style={{ ...finRowStyle, alignItems: 'center' }}
                      >
                        <span
                          style={{
                            fontSize: 12,
                        ...
                        <span
                          style={{ flex: 1, fontSize: 14, color: '#e2e8f0' }}
                        >
                          {o.note || (
                            <span
                        ...
                        <button
                          onClick={() =>
                            updateFinance(
                              'outings',
                              data.finance.outings.filter((x) => x.id !== o.id)
                            )
                          }
                          style={delBtn}
                        >
